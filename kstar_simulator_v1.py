...
from common.model_structure import k2rz, kstar_nn, kstar_v220505, tf_dense_model

# from common.model_structure import k2rz, x2rz, load_custom_model, kstar_lstm, kstar_v220505, kstar_nn, bpw_nn, tf_dense_model, actv, SB2_model, SB2_ensemble
from common.setting import rcParamsSetting
...
class KSTARWidget(QDialog):
    def __init__(self, parent=None):
        ...
        self.kstar_lstm = kstar_v220505(
            model_path=lstm_model_path, n_models=max_models
        )
    ...
    def predict0d(self, steady=True):
        ...
        if steady:
            ...
            self.x[:, 11 + 4] += (
                self.inputSliderDict[input_params[7]].value() / 10**decimals
            )
            self.x[:, 12 + 4] = 1 if self.x[-1, 12 + 4] > 1.265 + 1.0e-4 else 0
            self.x[:, -1] = year_in
            ...
            self.x[-1, 12 + 4] = (
                1 if self.x[-1, 12 + 4] > 1.265 + 1.0e-4 else 0
            )

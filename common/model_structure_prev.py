...
class k2rz:
    def __init__(
        ...
        self.models = [
            models.load_model(model_path + f"/best_model{i}", compile=False)
...
class kstar_lstm:
    def __init__(self, model_path, n_models=1, ymean=None, ystd=None):
        ...
        self.models = [
            load_custom_model(
                (10, 21), [200, 200], [200, 4], model_path + f"/best_model{i}"
            )
            for i in range(self.nmodels)
        ]
...
class SB2_model:
    def __init__(
        ...
        self.parameter_list = json.loads(
            zf.read("parameter_list").decode("utf-8")
        )
        self.parameters = np.load(zf.open("parameters"))
        self.layers = (
            data["policy_kwargs"]["layers"]
            if "layers" in data["policy_kwargs"].keys()
            else [64, 64]
        )
    ...
    def predict(self, x, yold=None):
        xnorm = (
            2
            * (x - self.low_state)
            / np.subtract(self.high_state, self.low_state)
            - 1
            if self.norm
            else x
        )
        ...
        for i, layer in enumerate(self.layers):
            w, b = (
                self.parameters[f"model/pi/fc{i}/kernel:0"],
                self.parameters[f"model/pi/fc{i}/bias:0"],
            )
            ynorm = actv(np.matmul(ynorm, w) + b, self.activation)
        w, b = (
            self.parameters[f"model/pi/dense/kernel:0"],
            self.parameters[f"model/pi/dense/bias:0"],
        )
        ynorm = actv(np.matmul(ynorm, w) + b, self.last_actv)

        y = (
            0.5 * np.subtract(self.high_action, self.low_action) * (ynorm + 1)
            + self.low_action
            if self.norm
            else ynorm
        )

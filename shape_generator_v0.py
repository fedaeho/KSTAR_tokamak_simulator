...
class PBGWidget(QDialog):
    ...
    def updateInputs(self):
        ...
        self.routvlabel.setText(
            f"{(self.routSlider.value()/10**decimals):.3f}"
        )
    ...
    def createPlotBox(self):
        ...
        self.fig = plt.figure(
            figsize=(2.5 * (100 / dpi), 4 * (100 / dpi)), dpi=dpi
        )
        self.plotPlasma()
    ...
    def reCreatePlotBox(self):
        ...
        self.mainLayout.replaceWidget(
            self.mainLayout.itemAtPosition(1, 1).widget(), self.plotBox
        )
    ...
    def dumpBoundary(self):
        ...
        print(
            "Ip [m]   Bt [T]   βp       Elon     Up.Tri   Lo.Tri   In.Mid.R Out.Mid.R"
        )
        print(
            f"{ip:.4f},  {bt:.4f},  {bp:.4f},  {k:.4f},  {du:.4f},  {dl:.4f},  {rin:.4f},  {rout:.4f}"
        )
        print("")
...
class k2rz:
    def __init__(
        ...
        for i in range(self.nmodels):
            self.models.append(
                models.load_model(
                    model_path + "/best_model{}".format(i),
                    custom_objects={"r2_k": self.r2_k},
                )
            )
